...
export default function ArticlePreview({ posts, roomTitle }: Props) {
  ...
  return (
    <>
      {showExport && (
        <ExportModal posts={posts} defaultTitle={roomTitle} onClose={() => setShowExport(false)} />
      )}
      ...
      <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`text-xs px-2 py-1 rounded border transition-colors ${
                cleanMode ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-500 hover:bg-gray-50'
              }`}
            >
              {cleanMode ? '详细' : '简洁'}
            </button>
            ...
            <button
              ...
              disabled={posts.length === 0}
              className="text-xs px-2 py-1 rounded border border-gray-200 text-gray-700 hover:bg-gray-50 disabled:opacity-40 font-medium"
            >
              导出 ↓
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {posts.length === 0 ? (
            ...
            <div className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{articleText}</div>
          ) : (
            <div className="space-y-3 text-sm leading-relaxed">
              {posts.map((post) => (
                <div key={post.id}>
                  <span className="text-xs text-gray-400 mr-1">[{post.author_nickname}]</span>
                  <span className={post.post_type === 'system' ? 'text-gray-400 italic' : 'text-gray-800'}>
